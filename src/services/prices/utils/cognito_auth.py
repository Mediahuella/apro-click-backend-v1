"""Auth ligera para el servicio prices.

Valida el Access Token de Cognito (``GetUser`` + ``AdminListGroupsForUser``)
sin tocar Postgres. Sirve para los endpoints administrativos donde sólo importa
"el usuario está autenticado y tiene un rol del staff".

Las operaciones de escritura (subir, activar, borrar listas) se restringen a
``SUPERADMIN`` / ``ADMIN`` desde el handler.
"""
...
def authenticate(access_token: str) -> dict[str, Any]:
    ...
    ``role`` es el primer grupo de Cognito en mayúsculas (precedencia más alta).
    ...
    if not role:
        raise PermissionError("Usuario sin rol válido para prices")
